...
## using different MERFISH slice as practice
...
#pointsI = np.array([[6793, 4604], [2417.0,6946.0]])
pointsI = np.array([[1207.0, 4396], [5293, 6604]])
...
for it in range(niter):
    ...
    if not it%10:
        ...
        ax[5].cla()
        ax[5].imshow(clip(fAI - J).permute(1,2,0).clone().detach().cpu()*0.5+0.5,extent=extentJ)
